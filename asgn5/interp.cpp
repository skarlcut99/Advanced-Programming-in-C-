...
void interpreter::do_border (param begin, param end) {
   ...
   window::border_width = from_string<float>(begin[1]);
}
...
shape_ptr interpreter::make_polygon (param begin, param end) {
   DEBUGF ('f', range (begin, end));
   vertex_list vert;
   for (int x = 0; begin + x != end; x += 2)
   {
        vertex tempver {from_string<GLfloat> (begin[x]),
                from_string<GLfloat> (begin[x+1])};
        vert.push_back(tempver);
   }
...
shape_ptr interpreter::make_triangle (param begin, param end) {
   ...
   return make_shared<triangle> (vert);
}
